...
class SelfAttention(nn.Module):
    def __init__(self, h_size):
        ...
        self.mha = nn.MultiheadAttention(h_size, 4)
...
class SAWrapper(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(-1, self.h_size, self.num_s * self.num_s).permute(0,2,1)
        x = self.sa(x)
        # x = x.swapaxes(2, 1).view(-1, self.h_size, self.num_s, self.num_s)
        x = x.permute(0,2,1).view(-1, self.h_size, self.num_s, self.num_s)
        return x
